...
import type { LezioniAdminFilters } from "@/lib/airtable-admin";
...
interface Props {
  initial: LezioniAdminFilters;
  anniDisponibili: number[];
  maestri: MaestroLite[];
}

export function LezioniFilters({ initial, anniDisponibili, maestri }: Props) {
  const router = useRouter();
  const pathname = usePathname();
  const searchParams = useSearchParams();

  const [search, setSearch] = React.useState(initial.bambinoSearch ?? "");
  ...
  const hasFilters =
    !!initial.anno || !!initial.mese || !!initial.maestroId || !!initial.bambinoSearch;

  const annoActive = initial.anno ?? null;
    ...
    <div className="bg-bg-soft border-b border-line py-3 z-30 sticky top-14">
      <div className="flex flex-col gap-2">
        <div className="flex flex-wrap items-center gap-2">
          ...
          {anniDisponibili.map((a) => (
            <button
              key={a}
              type="button"
              onClick={() => setParam("anno", annoActive === a ? null : String(a))}
              className={cn(
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <select
            value={initial.maestroId ?? ""}
            onChange={(e) => setParam("maestro", e.target.value || null)}
            className="h-9 px-3 text-sm border border-line rounded-[var(--radius-md)] bg-white text-ink focus:outline-none focus:ring-2 focus:ring-navy-700/20"
            aria-label="Maestro"
          >
            <option value="">Maestro: tutti</option>
            {maestri.map((m) => (
