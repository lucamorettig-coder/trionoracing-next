...
- **Data fine**: 2026-06-12
- **Stato**: completata (#1/#2 live; #3 attivo dopo allineamento env Clerk su Vercel)
...
### Esito (PR #67 — squash `8830b64` — live in produzione 2026-06-12)

Verifica ✅ con 1 ⚠️ residuo operativo (#3). I flussi auth-gated sono stati verificati via **smoke utente** (login Clerk non automatizzabile), non con `verify-implementation` automatico.
- **Quality gate**: `npm run build` ✅ · `npm run typecheck` exit 0 ✅ · `npm run lint` 0 errori ✅. Pagine pubbliche ancora `○ Static` (la navbar `useAuth()` non le rende dinamiche).
- **Smoke dev (utente)** ✅: login → `/portale`; navbar loggata mostra "Vai al portale"; crea figlio → home col banner conferma; header tariffe con colori vividi e leggibile.
- **Verifica prod post-deploy** ✅: deploy Vercel `success`; `/` 200 (navbar render), `/portale/login` 200, `/la-scuola` 200 — nessuna regressione pubblica.

**#1 e #2 live e funzionanti.** **#3**: codice in produzione, ma il redirect post-login si attiva **solo dopo** l'allineamento env Clerk su Vercel (⚠️ vedi sotto) — finché `NEXT_PUBLIC_CLERK_AFTER_SIGN_IN_URL=""` resta su Vercel, il login in prod torna alla landing. Analogo al promemoria GA4 di EVO-024.

### ⚠️ Residuo operativo (azione utente — attivazione #3 in prod)

Su **Vercel → trionoracing-next → Settings → Environment Variables** (Production + Preview):
- rimuovere/azzerare `NEXT_PUBLIC_CLERK_AFTER_SIGN_IN_URL` + `…AFTER_SIGN_UP_URL`
- aggiungere `NEXT_PUBLIC_CLERK_SIGN_IN_FALLBACK_REDIRECT_URL=/portale` + `…SIGN_UP_FALLBACK_REDIRECT_URL=/portale`
- valorizzare `NEXT_PUBLIC_CLERK_SIGN_IN_URL=/portale/login` + `…SIGN_UP_URL=/portale/registrati`
- **Redeploy** → verificare: login in prod atterra su `/portale`.

### Go-live

- **URL produzione**: https://trionoracing-next.vercel.app (portale `/portale`, admin tariffe `/portale/admin/tariffe`)
- **PR**: #67 (squash `8830b64`) · **Branch**: `evo-027-portale-ux-redirect-tariffe` (eliminato dopo merge)
- **Data go-live**: 2026-06-12 (#1/#2 immediati; #3 al completamento del residuo env)
...
### [2026-06-12] Fase 8 — Merge + go-live + consolidamento
PR #67 squash-merged (`8830b64`), branch eliminato. Deploy Vercel `success`, verifica prod ok (`/`, `/portale/login`, `/la-scuola` → 200). **#1/#2 live**; **#3** in attesa del residuo env Vercel (documentato). 4 pattern nuovi in AGENTS.md (token `--color-*`, redirect Clerk legacy, `useAuth` vs `SignedIn/SignedOut`, redirect-to-home). Doc di chiusura prodotti in **worktree isolato** (`evo-027-close`) perché il repo era passato a `feat/sumup-widget-telemetry` (lavoro parallelo utente) → nessuna interferenza col branch SumUp.
