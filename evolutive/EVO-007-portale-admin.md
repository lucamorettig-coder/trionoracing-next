...
|---|---|---|---|---|---|
| [EVO-016](EVO-016-admin-infra-ds.md) | `admin-infra-ds` | Scaffold DS (Dialog/AlertDialog/DropdownMenu/DataTable/AdminFilters/BulkActionBar/AdminPageHeader/ConfirmDialog/ExportCSVButton/KPICard) + skeleton `airtable-admin.ts` + estensione NavBar (11 link admin) + schema Airtable (campo `ANNULLATA` + formula estesa) + dashboard A-1 **minimal** (KPI + Today's tasks + quick actions) | ~3-4gg | 🔴 1 | nessuna |
| [EVO-017](EVO-017-admin-iscrizioni-bambini.md) | `admin-iscrizioni-bambini` | A-2 iscrizioni list + filtri + bulk + export CSV · A-3 dettaglio iscrizione con 5 tab (Stato/override · Modulistica · Taglie · Pagamenti admin · Storia) · A-4 bambini list + dettaglio · modal annulla/forza completa/aggiungi titolo manuale/segna pagato | ~5-6gg | 🔴 2 | EVO-016 |
